...
async def check_cita(provincia, tramite, oficina):
    ...
    target_url = f"https://icp.administracionelectronica.gob.es/icpplus/citar?p={pcode}&locale=es"
    
    # Use ScraperAPI if key available, otherwise try direct
    if SCRAPER_API_KEY:
        url = f"http://api.scraperapi.com?api_key={SCRAPER_API_KEY}&url={target_url}&country_code=es"
    else:
        url = target_url
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if r.status != 200:
                    logger.warning(f"Status {r.status} for {provincia}")
                    return None
                html = await r.text()

        if "host not in allowlist" in html.lower():
            logger.warning(f"Blocked for {provincia} - need ScraperAPI key")
            return None
        ...
        for sig in no_signals:
            if sig in html_lower:
                logger.info(f"No cita: {provincia} - {sig}")
                return None
        ...
        for sig in yes_signals:
            if re.search(sig, html_lower):
                logger.info(f"CITA FOUND: {provincia}/{tramite}")
                return {"provincia":provincia, "tramite":tramite,
                        "oficina":oficina, "url":target_url}
        return None
...
def tramite_keyboard(prov):
    ...
    for i, t in enumerate(tramites):
        ...
        # Use first 60 chars of tramite as callback data key
        cb_key = t[:60].replace(":", "-").replace("|", "-")
        kb.append([InlineKeyboardButton(label, callback_data=f"T:{cb_key}")])
    return InlineKeyboardMarkup(kb)
...
async def cb_tramite(u: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = u.callback_query; await q.answer()
    key = q.data[2:]  # first 60 chars of tramite
    prov = ctx.user_data.get("prov","?")
    # Find full tramite by matching prefix
    tramites = get_tramites(prov)
    tram = key  # fallback
    for t in tramites:
        cb_key = t[:60].replace(":", "-").replace("|", "-")
    ...
    logger.info(f"Tramite selected: {tram[:50]}")
    await q.edit_message_text(

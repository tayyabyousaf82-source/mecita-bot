...
# ══════════════════════════════════════════════════════════
# FLASK WEB SERVER (inline HTML - no templates needed)
# ══════════════════════════════════════════════════════════
flask_app = Flask(__name__)

@flask_app.route("/form")
def form():
    nie       = freq.args.get("nie", "")
    nombre    = freq.args.get("nombre", "")
    ano       = freq.args.get("ano", "")
    pais      = freq.args.get("pais", "")
    telefono  = freq.args.get("telefono", "")
    email     = freq.args.get("email", "")
    fecha_min = freq.args.get("fecha_min", "")
    fecha_max = freq.args.get("fecha_max", "")
    oficina   = freq.args.get("oficina", "")
    tramite   = freq.args.get("tramite", "")
    provincia = freq.args.get("provincia", "")
    ...
    paises_opts = "".join(
        f'<option value="{p}" {"selected" if p==pais else ""}>{p.title()}</option>'
        for p in paises
    )
...
<div class="section">
  ...
  <div style="font-size:11px;color:#888;margin-top:6px">{provincia} — {tramite}</div>
</div>
...
function renderCal(){{
  ...
  g.innerHTML='';
  ['lu','ma','mi','ju','vi','sá','do'].forEach(d=>{{const e=document.createElement('div');e.className='cal-day-name';e.textContent=d;g.appendChild(e);}});
...
function enviar(){{
  ...
  fetch('/submit',{{
    ...
    body:JSON.stringify({{nie,nombre:nom,ano,pais,telefono:tel,
      ...
      oficina:'{oficina}',tramite:'{tramite}',provincia:'{provincia}',
...
@flask_app.route("/")
def home():
    return "MiCitaBot is running!", 200

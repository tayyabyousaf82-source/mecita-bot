...
class CustomerProfile:
    ...
    anticaptcha_api_key: Optional[str] = None  # holds 2captcha key for compatibility
...
def init_wedriver(context: CustomerProfile):
    ...
    if os.environ.get("HEADLESS", "true").lower() in ["1", "true", "yes", "on"]:
    ...
    chrome_driver_path = os.environ.get("CHROMEDRIVER_PATH", context.chrome_driver_path)
...
def solve_recaptcha(driver: webdriver, context: CustomerProfile):
    ...
    logging.info("2Captcha: site key: " + site_key)
    logging.info("2Captcha: action: " + page_action)

    try:
        solver = TwoCaptcha(context.anticaptcha_api_key)
        ...
        logging.info("2Captcha: g-response: " + g_response)
        driver.execute_script(
            f"document.getElementById('g-recaptcha-response').value = '{g_response}'"
        )
        return True
    except Exception as e:
        logging.error("2Captcha recaptcha error: " + str(e))
        return None
...
def solve_image_captcha(driver: webdriver, context: CustomerProfile):
    try:
        ...
        if captcha_result:
            logging.info("2Captcha: captcha text: " + captcha_result)
            element = driver.find_element(By.ID, "captcha")
            element.send_keys(captcha_result)
            return True
        else:
            logging.error("2Captcha: empty result for image captcha")
            return None
    except Exception as e:
        logging.error("2Captcha image captcha error: " + str(e))
        return None
    finally:
        os.unlink(tmp.name)
...
def confirm_appointment(driver: webdriver, context: CustomerProfile):
    ...
    if "CITA CONFIRMADA Y GRABADA" in resp_text:
        ...
        if context.telegram_token and context.telegram_chat_id:
            try:
                msg = f"✅ CITA CONFIRMADA!\nJustificante: {code}\nNombre: {context.name}\nFecha: {ctime.strftime('%d/%m/%Y %H:%M')}"
                requests.get(
                    f"https://api.telegram.org/bot{context.telegram_token}/sendMessage",
                    params={"chat_id": context.telegram_chat_id, "text": msg},
                    timeout=10,
                )
